...
- ~~xformers 削除（SDPA 移行）の可否判断~~: 動作確認後に削除を決定し、実施済み
  （`enable_xformers_memory_efficient_attention()` 2 箇所の削除と依存除去 + 再ロック）。
  これにより torch と xformers の足並み問題自体が解消した
- ~~モデル切り替えが頻繁になる場合は、app_vid.py と同様の `modal.Volume` 方式への
  移行を検討~~: 2026-07-11_sdxl-model-volume.md のプランで実施済み
  （複数モデル併存・再ダウンロード不要・`modal volume` CLI で管理可能）
